  ...
  return (
    <div className="bg-[#f7f4ef] min-h-screen">
      <div className="pt-32 pb-14 px-6 text-center">
        ...
        <h1 className="font-serif text-5xl lg:text-6xl text-[#0F4C81] mb-5">Gallery</h1>
          ...
          <TabsList className="flex flex-wrap justify-center gap-1 bg-transparent mb-12 h-auto">
            {categories.map(({ value, label }) => (
              <TabsTrigger
                key={value}
                value={value}
                className="text-[10px] tracking-[0.2em] uppercase font-sans px-5 py-2.5 rounded-none border border-[#d6d0c6] bg-transparent text-[#6b7869] data-[state=active]:bg-[#0F4C81] data-[state=active]:text-[#f7f4ef] data-[state=active]:border-[#0F4C81] hover:border-[#0F4C81] hover:text-[#0F4C81] transition-all"
              >
                {label}
              </TabsTrigger>
            ))}
